...
public static class MapGenerator
{
    ...
    private static void CreateForests(MapGraph graph, float preferredElevation, float heightDifferenceWeighting, int maxForests)
    {
        int forestCount = 0;
        while (forestCount < maxForests) {
            var candidate = GetForestCandidate(graph, preferredElevation, heightDifferenceWeighting).FirstOrDefault();
            if (candidate == null) break;
            candidate.nodeType = MapGraph.MapNodeType.Forest;
            forestCount++;
        }
    }

    private static IOrderedEnumerable<MapGraph.MapNode> GetForestCandidate(MapGraph graph, float preferredElevation, float heightDifferenceWeighting)
    {
    ...
    private static void SetLowNodesToWater(MapGraph graph, float cutoff)
    {
        foreach (var node in graph.nodesByCenterPosition.Values) {
            if (node.centerPoint.y <= cutoff) {
                ...
                if (allZero && node.nodeType != MapGraph.MapNodeType.Error) 
                    node.nodeType = MapGraph.MapNodeType.FreshWater;
            }
        }
    }
